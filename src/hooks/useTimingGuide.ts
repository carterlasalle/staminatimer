...
export function useTimingGuide() {
  ...
  const getStrokePhaseInfo = useCallback(() => {
    ...
    const secondsPerCount =
      strokeConfig.totalDuration / (strokeConfig.upCount + strokeConfig.downCount)
  ...
  useEffect(() => {
    ...
    const initialDuration =
      sessionPhase === 'on' ? sessionConfig.onDuration : sessionConfig.offDuration
  ...
  const changeStrokeLevel = useCallback(
    (level: StrokeTimingLevel) => {
  ...
  const changeSessionInterval = useCallback(
    (interval: SessionInterval) => {
