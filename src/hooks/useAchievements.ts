    ...
    async (session: DBSession) => {
      ...
      for (const achievement of achievements) {
        ...
        switch (achievement.condition_type) {
          case 'duration':
            progress = Math.round(
              Math.min(100, (session.total_duration / achievement.condition_value) * 100)
            )
            unlocked =
              achievement.condition_comparison === 'greater'
          ...
          case 'edge_duration': {
            const maxEdgeDuration = Math.max(
              ...(session.edge_events?.map((e) => e.duration ?? 0) ?? [0])
            )
            progress = Math.round(
              Math.min(100, (achievement.condition_value / maxEdgeDuration) * 100)
            )
            unlocked =
              achievement.condition_comparison === 'less'
          ...
          case 'streak':
            if (historicalSessions) {
              ...
              progress = Math.round(
                Math.min(100, (currentStreak / achievement.condition_value) * 100)
              )
                ...
                if (historicalSessions && historicalSessions.length > 1) {
                  const oldAvg =
                    historicalSessions
                      .slice(1)
                      .reduce(
                        (acc: number, s: { active_duration: number }) => acc + s.active_duration,
                        0
                      ) /
