...
export function useAICoach() {
  ...
  const generateComprehensivePrompt = useCallback(
    (userMessage: string) => {
      ...
      const safeMessage = flagged ? 'Please provide general training advice.' : sanitizedMessage
...
${recentSessions
  ...
  .map(
    (session, i) =>
      `${i + 1}. ${new Date(session.created_at).toLocaleDateString()} - ${Math.round((session.total_duration || 0) / 60000)}m ${Math.round(((session.total_duration || 0) % 60000) / 1000)}s | ${session.edge_events?.length || 0} edges | ${!session.finished_during_edge ? 'Success' : 'Edge finish'}`
  )
  .join('\n')}
  ...
  const sendMessage = useCallback(
    async (userMessage: string) => {
      ...
      try {
        ...
        let errorContent =
          'I apologize, but I encountered an error while analyzing your data. Please try again.'
  ...
  const generateInitialInsights = useCallback(async () => {
    ...
    try {
      const prompt = generateComprehensivePrompt(
        'Please analyze my complete training data and provide a comprehensive performance assessment with personalized recommendations for improvement. I want to understand my progress patterns, areas for improvement, and get specific next steps for advancing my stamina training.'
      )
      const response = await generateAIResponse(prompt)
      ...
      let errorContent =
        'I encountered an error while analyzing your training data. Please try again.'
      const errorMessage = error instanceof Error ? error.message : ''
...
function calculateAdvancedStats(sessions: DBSession[]) {
  if (!sessions.length) {
    return {
      ...
      patterns:
        'Insufficient data for pattern analysis. Complete more training sessions for detailed insights.',
    }
  }

  // Calculate average duration
  const avgDurationMs =
    sessions.reduce((acc, s) => acc + (s.total_duration || 0), 0) / sessions.length
  ...
  const avgEdgeTime =
    sessions.reduce((acc, s) => acc + (s.edge_duration || 0), 0) / sessions.length / 1000
  ...
  for (let i = 0; i < sessions.length - 1; i++) {
    const diff =
      new Date(sessions[i].created_at).getTime() - new Date(sessions[i + 1].created_at).getTime()
    daysBetweenSessions.push(Math.floor(diff / (1000 * 60 * 60 * 24)))
  }

  const avgDaysBetween =
    daysBetweenSessions.reduce((acc, days) => acc + days, 0) / daysBetweenSessions.length
  ...
  const peakSessions = sessions.filter(
    (s) => (s.total_duration || 0) > avgDurationMs * 1.5 && !s.finished_during_edge
  ).length
  ...
  const sessionsPerWeek =
    totalDays > 0 ? Math.round((sessions.length / totalDays) * 7 * 10) / 10 : 0
  ...
  if (successRate < 50)
    patterns.push(
      'Pattern: Frequent edge finishes indicate need for enhanced edge recognition and control techniques'
    )

  if (avgEdgeTime > 90)
    patterns.push(
      'Pattern: Extended edge periods suggest need for breathing and mindfulness technique refinement'
    )

  if (consistencyScore < 60)
    patterns.push(
      'Pattern: Irregular training frequency may be limiting progress - consistency is key for neuroplasticity development'
    )

  if (sessions.some((s) => (s.edge_events?.length || 0) > 5))
    patterns.push(
      'Pattern: High number of edge events in some sessions indicate good awareness but may suggest training intensity adjustments needed'
    )
  ...
  return {
    ...
    performanceComparison:
      recentThree.length >= 3
        ? `Recent performance: ${Math.round((recentThree.filter((s) => !s.finished_during_edge).length / 3) * 100)}% success rate`
