...
export function useTimer() {
  ...
  const endEdge = useCallback(async () => {
    ...
    try {
      ...
      setEdgeLaps((prev) => {
        const next = [...prev]
        const lap = next[next.length - 1]

        if (lap) next[next.length - 1] = { ...lap, endTime: now, duration: lapDuration }
  ...
  const finishSession = useCallback(async () => {
    ...
    try {
      ...
      const rpcArgs: FinishTimerRpcArgs = {
        ...
        p_active_duration: finalActiveTime,
        p_edge_duration: finalEdgeTime,
        p_total_duration: finalActiveTime + finalEdgeTime,
        p_finished_during_edge: finalFinishedDuringEdge,
        p_open_edge_duration: openEdgeDuration,
      }

      const { error } = await (
        supabase.rpc as unknown as (
          fn: string,
          args: FinishTimerRpcArgs
        ) => Promise<{ error: unknown }>
      )('finish_timer_session', rpcArgs)

      if (error) throw error
      setActiveTime(finalActiveTime)
      setDisplayActiveTime(finalActiveTime)
      setEdgeTime(finalEdgeTime)
      setDisplayEdgeTime(finalEdgeTime)
      ...
      if (finalFinishedDuringEdge && openEdgeDuration !== null) {
        setEdgeLaps((prev) => {
          ...
          if (lap) next[next.length - 1] = { ...lap, endTime: now, duration: openEdgeDuration }
