        ...
        context = gsap.context(() => {
          gsap
            ...
            .fromTo(
              '.progress-baseline',
              { scaleX: 0 },
              { scaleX: 1, duration: 0.65, transformOrigin: 'left', ease: 'none' }
            )
            .to(path, { strokeDashoffset: 0, duration: 1.55, ease: 'none' }, 0.18)
            .fromTo(
              '.progress-mark',
              { scale: 0, opacity: 0 },
              { scale: 1, opacity: 1, stagger: 0.11, duration: 0.25, ease: 'none' },
              0.8
            )
            .fromTo(
              '.progress-target-line',
              { scaleY: 0 },
              { scaleY: 1, duration: 0.55, transformOrigin: 'bottom', ease: 'none' },
              1.1
            )
        }, section)
      })
  ...
  return (
    <section ref={sectionRef} id="progress" className="landing-progress-section">
      <div className="landing-progress-copy">
        <p className="text-sm text-landing-soft">Targets are earned.</p>
        <h2 className="mt-4 max-w-lg font-display text-5xl leading-[0.92] tracking-[-0.06em] text-landing-paper sm:text-7xl">
          Progress only moves when performance repeats.
        </h2>
        <p className="mt-6 max-w-md text-base leading-relaxed text-landing-soft">
      ...
      <div className="landing-ridge-chart" aria-label="Example progression from two to ten minutes">
        <svg viewBox="0 0 1200 500" className="h-auto w-full overflow-visible" role="img">
          ...
          <line
            ...
            stroke="var(--landing-sun)"
            strokeWidth="2"
          />
          <g fill="var(--landing-sun)">
            {[
              ['202', '348'],
              ['498', '252'],
              ['668', '199'],
        ...
        <div className="mt-5 grid grid-cols-4 gap-x-3 gap-y-2 border-t border-landing-soft/50 pt-4 text-xs text-landing-soft sm:grid-cols-6 lg:grid-cols-12">
          {TARGETS.map((target) => (
            <span key={target} className={target === '5:00' ? 'text-landing-sun' : ''}>
              {target}
            </span>
          ))}
        </div>
