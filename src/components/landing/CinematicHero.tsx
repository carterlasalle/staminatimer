    ...
    void import('@/lib/motion/gsap-runtime').then(async ({ withGsap }) => {
      ...
      await withGsap(({ gsap }) => {
        ...
        context = gsap.context(() => {
          gsap.fromTo(scene, { scale: 1.012 }, { scale: 1, duration: 1.15, ease: 'none' })
          gsap.fromTo(
            '.hero-arrival',
            { y: 12, opacity: 0 },
            { y: 0, opacity: 1, stagger: 0.1, duration: 0.7, delay: 0.18, ease: 'none' }
          )
          gsap
            .timeline({
              scrollTrigger: {
    ...
    <section ref={heroRef} className="landing-hero-wrap" aria-label="Stamina Timer introduction">
      <div ref={sceneRef} className="landing-hero-scene">
        ...
        <div className="landing-hero-copy">
          ...
          <h1 className="landing-hero-title mt-4 max-w-[8ch] font-display text-[clamp(3.4rem,7.4vw,8.2rem)] font-semibold leading-[0.87] tracking-[-0.07em] text-landing-paper">
            Build control
            <br />
            that lasts.
          </h1>
          <p className="hero-arrival mt-7 max-w-md text-base leading-relaxed text-landing-mist sm:text-lg">
