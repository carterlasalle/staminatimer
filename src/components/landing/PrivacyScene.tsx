  ...
  return (
    <section id="privacy" className="landing-privacy-section">
      ...
      <div className="landing-privacy-content">
        <p className="landing-kicker text-[#75a3b2]">Private by design</p>
        <h2 className="mt-5 max-w-xl font-display text-5xl leading-[0.92] tracking-[-0.06em] text-[#edf3f1] sm:text-7xl">
          Your training stays yours.
        </h2>
        <dl className="mt-16 border-y border-[#d7e5e5]/15">
          {PRIVACY_POINTS.map(([term, description]) => (
            <div
              key={term}
              className="grid gap-2 border-b border-[#d7e5e5]/15 py-5 last:border-0 sm:grid-cols-[minmax(11rem,.55fr)_1fr]"
            >
              <dt className="font-medium text-[#edf3f1]">{term}</dt>
              <dd className="text-sm leading-relaxed text-[#afc9cf]">{description}</dd>
            </div>
          ))}
        </dl>
        <div className="mt-24 flex flex-wrap items-end justify-between gap-8">
          <div>
            <p className="text-sm text-[#75a3b2]">A quieter place to begin.</p>
            <h3 className="mt-3 font-display text-4xl tracking-[-0.05em] text-[#edf3f1] sm:text-5xl">
              Ready when you are.
            </h3>
          </div>
