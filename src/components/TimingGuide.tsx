...
import {
  useTimingGuide,
  type StrokeTimingLevel,
  type SessionInterval,
} from '@/hooks/useTimingGuide'
import { Activity, ArrowUp, ArrowDown, Play, Pause, RotateCcw, Timer, Zap } from 'lucide-react'
          ...
          {isStrokeGuideEnabled && (
            <div className="space-y-4 p-4 bg-primary/10 rounded-lg border border-primary/20">
              ...
              <div className="flex justify-center">
                <Button variant="outline" size="sm" onClick={resetStrokeGuide} className="gap-2">
            ...
            >
              ...
              <SelectContent>
                {sessionIntervals.map((interval) => {
                  const config = {
                    '20s/40s': { on: 20, off: 40 },
                    '30s/60s': { on: 30, off: 60 },
                    '45s/90s': { on: 45, off: 90 },
                    '60s/120s': { on: 60, off: 120 },
                  }[interval]
          ...
          {isGuidedSessionEnabled && (
            <div className="space-y-4 rounded-lg border border-warning/40 bg-warning/10 p-4">
              <div className="text-center">
                ...
                <div className="text-2xl font-mono mb-2">{formatTime(sessionTimeRemaining)}</div>
                  ...
                  value={
                    sessionPhase === 'on'
                      ? ((sessionConfig.onDuration - sessionTimeRemaining) /
                        ...
                        100
                      : ((sessionConfig.offDuration - sessionTimeRemaining) /
                          sessionConfig.offDuration) *
                        100
                  }
                  className="h-2"
                />
              ...
              <div className="flex justify-center">
                <Button variant="outline" size="sm" onClick={resetGuidedSession} className="gap-2">
                  <RotateCcw className="h-4 w-4" />
                  Reset
                </Button>
              </div>
        ...
        <CardContent className="space-y-3 text-sm">
          <div>
            ...
            <p className="text-muted-foreground">
              Follow the up/down count to maintain consistent rhythm. Start with beginner (3 up, 3
              down) and progress to advanced (1 up, 1 down) as your control improves.
            </p>
          </div>
          <div>
            <h4 className="font-medium mb-1">Guided Sessions:</h4>
            <p className="text-muted-foreground">
              Alternate between active stimulation and rest periods. Use 30s/60s for building
              endurance, or 20s/40s for control training.
            </p>
          </div>
        </CardContent>
      </Card>
