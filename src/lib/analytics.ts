...
export function calculateDetailedAnalytics(sessions: DBSession[]): DetailedAnalytics {
  ...
  const validSessions = sessions.filter(
    (s) => typeof s.total_duration === 'number' && typeof s.edge_duration === 'number'
  )
  ...
  return {
    averageSessionDuration:
      validSessions.reduce((acc, s) => acc + s.total_duration, 0) / validSessions.length,
    averageEdgeDuration:
      totalEdges > 0
...
function calculateImprovementRate(sessions: DBSession[]): number {
  ...
  const recentAvgDuration =
    recent3Sessions.reduce((acc, s) => acc + (s.total_duration ?? 0), 0) / recent3Sessions.length

  // Calculate average total duration for previous 3 sessions
  const previousAvgDuration =
    previous3Sessions.reduce((acc, s) => acc + (s.total_duration ?? 0), 0) /
    previous3Sessions.length
